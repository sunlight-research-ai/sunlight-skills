...
4. Have each investigator run iterative research: use the model's default `web_search` first, use all available Tavily, Exa, and Linkup providers when their API keys/tools are available, merge and deduplicate sources, inspect sources, capture source-tagged findings, reflect on missing evidence, then stop when coverage or budget is reached.
